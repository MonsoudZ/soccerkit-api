...
| **me** | `GET /me` — the authenticated person + their org memberships, `DELETE /me` (full account erasure), `GET /me/teams` — the teams you are rostered on, with jersey number, position and dates, `GET /me/children` — the children you are a recorded guardian of. That last one is the first call a parent's app can make: guardianships were readable only from the child's side (`GET /persons/:id/guardians`, which needs the id you are trying to find), so a parent had to walk `GET /teams` and a team roster to discover their own children, and a child on no roster could not be found at all. Not org-scoped — a guardianship is a fact about two people rather than about a club |
...
| **content** | `GET/POST /drills`, `GET/POST /sessions`, `GET/PATCH/DELETE /sessions/:id` (sessions carry ordered blocks that can reference drills). Reading training is role-scoped rather than staff-only: staff see the organization's, a player their teams', a parent their children's — and a family gets the session without its blocks. The coaching library is the drills, the plan and the templates, and it stays closed; that a squad trains at six on Tuesday is logistics, and hiding it from the people expected there was that instinct applied one level too wide — it also left the training reminder deep-linking to a 403 for exactly the families it was addressed to. A session with no team is on nobody's calendar but its author's, and an id the caller may not see answers 404 rather than 403. `PATCH` moves, renames and re-notes a session — it was the one scheduled thing that could be created and deleted but never edited, so moving Tuesday training to Thursday meant deleting it and rebuilding, which now throws away the register too. A changed time pushes to the squad, the same as a moved kickoff; a rename does not. Blocks are not editable there: replacing the plan is a different operation from editing the session, and a caller sending none would erase one. Changing the team is refused once the register has been answered — those replies are about a specific squad's training and cannot be carried onto another's |
